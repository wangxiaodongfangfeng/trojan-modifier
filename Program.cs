...
builder.Services.AddCors(options =>
{
    options.AddPolicy(corsPolicy,
        policy =>
        {
            policy.WithOrigins(origin.Split(',')).AllowAnyMethod().AllowAnyHeader().AllowCredentials();
        });
});
...
var modifier = new ConfigModifier("/usr/src/trojan/config.json");
app.MapGet("/config", () =>
    {
...
app.MapPost("/config", (ConfigItem item) =>
    {
        if (item is { Ip: not null, Password: not null, Port: not null })
        {
        ...
        {
            return Results.Problem("should give enough information");
        }
    })
